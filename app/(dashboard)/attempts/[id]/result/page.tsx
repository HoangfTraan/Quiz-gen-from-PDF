...
export default async function ResultPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: answers } = await supabase
    ...
    .select(`
       id,
       is_correct,
       questions (id, question_text, explanation),
       question_options (*),
       selected_option_id
    `)
    .eq('attempt_id', id);
        ...
        {answers?.map((ans: any, index: number) => {
           ...
           return (
             <div key={ans.id} className={`bg-white p-6 rounded-2xl shadow-sm relative border-l-4 ${ans.is_correct ? 'border-green-500' : 'border-red-500'} overflow-hidden`}>
               ...
               <h3 className="font-bold text-lg mb-5 text-gray-800 pr-12 leading-relaxed">Câu {index + 1}: {ans.questions?.question_text}</h3>
               ...
               <div className="bg-gray-50 p-4 rounded-xl text-sm text-gray-600 border border-gray-100 leading-relaxed">
                  ...
                  {ans.questions?.explanation || 'Không có giải thích cho câu hỏi này.'}
               </div>
             </div>
           );
        })}
