    ...
    if (errorText) {
      ...
      const dismissTimer = setTimeout(() => {
        ...
        const clearTimer = setTimeout(() => {
          setErrorText("");
          setIsErrorExiting(false);
        }, 400); // 400ms matching the slide-up-out animation duration
    ...
    const handleDrop = (e: DragEvent) => {
      ...
      if (e.dataTransfer?.files && e.dataTransfer.files.length > 0) {
         ...
         if (selectedFile.type !== "application/pdf" && !selectedFile.name.toLowerCase().endsWith(".pdf")) {
           setErrorText("Hệ thống chỉ hỗ trợ định dạng tệp PDF. Vui lòng chọn tệp tin PDF hợp lệ!");
           setFile(null);
           return;
         }
      ...
      <div className="bg-white p-8 rounded-xl shadow-lg border border-gray-100">
        {errorText && (
          <div className={`mb-4 p-3 bg-red-50 text-red-700 border border-red-200 rounded-lg flex items-center gap-2 text-sm ${
            ...
            <AlertCircle size={16} className={isErrorExiting ? "" : "animate-pulse"} /> {errorText}
          </div>
        )}
        ...
        <div className="w-full p-8 border-2 border-dashed border-blue-200 bg-blue-50 rounded-lg hover:border-blue-400 hover:bg-blue-100 transition-all group mb-8 flex flex-col items-center justify-center relative">
          <input type="file" accept=".pdf" onChange={handleFileChange} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" id="file-upload" />
          <div className="flex flex-col items-center justify-center text-blue-600 pointer-events-none">
            ...
            <span className="font-semibold text-center mt-2 text-gray-800">
               {file ? <span className="text-green-600">{file.name}</span> : "Kéo thả hoặc click chọn file (PDF)"}
            </span>
