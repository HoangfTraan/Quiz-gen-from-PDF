...
import { History as HistoryIcon, Clock, Award, GraduationCap } from "lucide-react";
import { createClient } from "@/utils/supabase/server";
import { getUserRole } from "@/utils/rbac-server";
import { canViewScores } from "@/utils/rbac";
...
export default async function HistoryPage(props: {
  ...
  const hasAccess = canViewScores(role);

  // Nếu không phải learner → hiển thị trang khoá thay vì redirect
  if (!hasAccess) {
    return (
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="min-w-full divide-y divide-gray-200">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody
              key={page}
              className="bg-white divide-y divide-gray-200 animate-page-fade"
            >
              {history.length === 0 ? (
                <tr>
                  <td
                  ...
                  >
                    ...
                    <Link
                      href="/quizzes"
                      className="text-blue-600 hover:outline-none hover:underline font-bold"
                    >
                ...
                history.map((record) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span
                          className={`px-3 py-1 inline-flex text-xs font-black rounded-full border items-center gap-1.5 ${
                            ratio >= 0.8
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        <Link
                          ...
                          className="text-blue-600 hover:text-blue-800 font-bold underline decoration-blue-200 underline-offset-4 decoration-2 transition-all"
                        >
                          Xem chi tiết
                        </Link>
                      </td>
